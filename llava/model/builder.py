...
def handle_fga(model, compute_dtype, device):
    ...
    model.initialize_fga(util_e, sharing_factor, False, sizes, size_force=False, similar_modalities=similar_modalities).to(dtype=compute_dtype, device=device)
...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", device="cuda", use_flash_attn=False, **kwargs):
    ...
    if 'llava' in model_name.lower():
        ...
        if 'lora' in model_name.lower() and model_base is not None:
            ...
            prior_keys = list(model.state_dict().keys())
            ...
            missing_keys_except_the_loaded = [key for key in incompat.missing_keys if key not in prior_keys]
            for key in missing_keys_except_the_loaded:
                print(" -", key)
